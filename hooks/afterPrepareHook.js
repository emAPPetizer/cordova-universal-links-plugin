 ...
 * @return {Promise} resolves when all platforms are processed
 */
function run(cordovaContext) {
  ...
  return Promise.all(
    platformsList.map(function (platform) {
      switch (platform) {
        case ANDROID:
          return activateUniversalLinksInAndroid(
            cordovaContext,
            pluginPreferences,
          );
        case IOS:
          return activateUniversalLinksInIos(cordovaContext, pluginPreferences);
      }
    }),
  );
}
...
function activateUniversalLinksInIos(cordovaContext, pluginPreferences) {
  ...
  return iosProjectEntitlements.generateAssociatedDomainsEntitlements(
    cordovaContext,
    pluginPreferences,
  );
}
